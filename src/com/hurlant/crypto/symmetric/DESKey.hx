...
class DESKey implements ISymmetricKey
{
    ...
    private static var Df_Key : Array<Dynamic> = [0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef, 0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54, 0x32, 
        0x10, 0x89, 0xab, 0xcd, 0xef, 0x01, 0x23, 0x45, 0x67];
    
    private static var bytebit : Array<Dynamic> = [128, 64, 32, 16, 8, 4, 2, 1];
    
    private static var bigbyte : Array<Dynamic> = [0x800000, 0x400000, 0x200000, 0x100000, 0x80000, 0x40000, 0x20000, 0x10000, 0x8000, 
    ...
    private static var pc1 : Array<Dynamic> = [56, 48, 40, 32, 24, 16, 8, 0, 57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34, 26, 18, 10, 2, 
        59, 51, 43, 35, 62, 54, 46, 38, 30, 22, 14, 6, 61, 53, 45, 37, 29, 21, 13, 5, 60, 52, 44, 36, 28, 20, 12, 
        4, 27, 19, 11, 3];
    
    private static var totrot : Array<Dynamic> = [1, 2, 4, 6, 8, 10, 12, 14, 15, 17, 19, 21, 23, 25, 27, 28];
    
    private static var pc2 : Array<Dynamic> = [13, 16, 10, 23, 0, 4, 2, 27, 14, 5, 20, 9, 22, 18, 11, 3, 25, 7, 15, 6, 26, 19, 12, 1, 40, 
        51, 30, 36, 46, 54, 29, 39, 50, 44, 32, 47, 43, 48, 38, 55, 33, 52, 45, 41, 49, 35, 28, 31];
    
    private static var SP1 : Array<Dynamic> = [0x01010400, 0x00000000, 0x00010000, 0x01010404, 0x01010004, 0x00010404, 0x00000004, 
    ...
    private static var SP2 : Array<Dynamic> = [0x80108020, 0x80008000, 0x00008000, 0x00108020, 0x00100000, 0x00000020, 0x80100020, 
    ...
    private static var SP3 : Array<Dynamic> = [0x00000208, 0x08020200, 0x00000000, 0x08020008, 0x08000200, 0x00000000, 0x00020208, 
    ...
    private static var SP4 : Array<Dynamic> = [0x00802001, 0x00002081, 0x00002081, 0x00000080, 0x00802080, 0x00800081, 0x00800001, 
    ...
    private static var SP5 : Array<Dynamic> = [0x00000100, 0x02080100, 0x02080000, 0x42000100, 0x00080000, 0x00000100, 0x40000000, 
    ...
    private static var SP6 : Array<Dynamic> = [0x20000010, 0x20400000, 0x00004000, 0x20404010, 0x20400000, 0x00000010, 0x20404010, 
    ...
    private static var SP7 : Array<Dynamic> = [0x00200000, 0x04200002, 0x04000802, 0x00000000, 0x00000800, 0x04000802, 0x00200802, 
    ...
    private static var SP8 : Array<Dynamic> = [0x10001040, 0x00001000, 0x00040000, 0x10041040, 0x10000000, 0x10001040, 0x00000040, 
    ...
    private function generateWorkingKey(encrypting : Bool, key : ByteArray, off : Int) : Array<Dynamic>
    {
        ...
        for (j in 0...56){
            ...
            pc1m[j] = ((key.get(off + (l >>> 3)) & bytebit[l & 7]) != 0) ? 1 : 0;
        }
        ...
        for (i in 0...16){
            ...
            for (j in 0...24){
                if (Reflect.field(pcr, Std.string(pc2[j]))) 
                {
                    newKey[m] |= bigbyte[j];
                }
                
                if (Reflect.field(pcr, Std.string(pc2[j + 24]))) 
                {
                    newKey[n] |= bigbyte[j];
                }
            }
