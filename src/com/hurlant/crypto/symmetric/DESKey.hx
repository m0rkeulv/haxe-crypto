...
class DESKey implements ISymmetricKey {
    ...
    private static var Df_Key = [
        0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef, 0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54, 0x32,
        0x10, 0x89, 0xab, 0xcd, 0xef, 0x01, 0x23, 0x45, 0x67];

    private static var bytebit = [128, 64, 32, 16, 8, 4, 2, 1];

    private static var bigbyte = [
        0x800000, 0x400000, 0x200000, 0x100000, 0x80000, 0x40000, 0x20000, 0x10000, 0x8000,
    ...
    private static var totrot = [1, 2, 4, 6, 8, 10, 12, 14, 15, 17, 19, 21, 23, 25, 27, 28];

    private static var pc2 = [
        13, 16, 10, 23, 0, 4, 2, 27, 14, 5, 20, 9, 22, 18, 11, 3, 25, 7, 15, 6, 26, 19, 12, 1, 40,
        51, 30, 36, 46, 54, 29, 39, 50, 44, 32, 47, 43, 48, 38, 55, 33, 52, 45, 41, 49, 35, 28, 31
    ];

    private static var SP1 = [
    ...
    private function generateWorkingKey(encrypting:Bool, key:ByteArray, off:Int):Array<Dynamic> {
        ...
        for (i in 0...16) {
            ...
            for (j in 0 ... 24) {
                if (pcr[pc2[j +  0]] != 0) newKey[m] |= bigbyte[j];
                if (pcr[pc2[j + 24]] != 0) newKey[n] |= bigbyte[j];
            }
        }
